...
// remove medic euqipment from unconscious unit
	...
	if (isPlayer _unit) then {
		...
		// Count Medkits
			...
			_medicalStore = _unit getVariable "AIS_MedicalStore";
			_countAllMedikits = (_medicalStore select 0) + (_medicalStore select 1) + (_medicalStore select 2);
	
		if (_countAllMedikits > 2) then {
		
			// Withstand

				player setVariable ["canWithStand", TRUE];
