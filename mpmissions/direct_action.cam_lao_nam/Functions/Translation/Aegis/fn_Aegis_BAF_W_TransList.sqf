
	// Units
	
		_rifleman				= 	"B_A_Soldier_wdl_F";
		_officer 				=	"B_A_Officer_wdl_F";
		_teamLeader 			= 	"B_A_Soldier_TL_wdl_F";
		_marksman 				= 	"B_A_soldier_M_wdl_F";
		_sniper					=	"B_A_ghillie_wdl_F";
		_spotter				=	"B_A_ghillie_spotter_wdl_F";
		_grenadier				=	"B_A_Soldier_GL_wdl_F";
		_autoRifleman			= 	"B_A_Soldier_AR_wdl_F";
		_gunner					=	"B_A_HeavyGunner_wdl_F";
		_medic					= 	"B_A_Medic_wdl_F";
		_repairSpecialist		=	"B_A_Soldier_Repair_wdl_F";
		_engineer				= 	"B_A_Engineer_wdl_F";
		_guidedLAT				=	"B_A_Soldier_LAT_wdl_F";
		_dumbLAT				=	"B_A_Soldier_LAT_wdl_F";
		_missileSpecialistAT	= 	"B_A_Soldier_AT_wdl_F";
		_missileSpecialistAA	=	"B_A_Soldier_AA_wdl_F";
		_ammoBearer				= 	"B_A_Soldier_A_wdl_F";
		_JTAC					= 	"B_A_RadioOperator_wdl_F";
		_explosivesSpecialist	= 	"B_A_Soldier_Exp_wdl_F";
		_uavOperator			= 	"B_A_Soldier_UAV_wdl_F";
		_paratrooper			=	"B_A_Soldier_PG_wdl_F";
		_heliPilot 				=	"B_A_Helipilot_wdl_F";
		_heliCrew				=	"B_A_Helicrew_wdl_F";
		_fighterPilot			=	"B_A_Fighter_Pilot_wdl_F";
		_pilot					=	"B_A_Pilot_wdl_F";
		_vehicleCrew			=  	"B_A_Crew_wdl_F";
		_diver 					=	"B_A_Diver_F";
		_diverTeamLeader		=	"B_A_Diver_TL_F";
		_diverExplosives		=	"B_A_Diver_Exp_F";

	
	// Vehicles
	
		_atTurret				=	"B_static_AT_F";
		_aaTurret				=	"B_static_AA_F";
		_mortar					=	"B_Mortar_01_F";
		_AttackCargoPlane		=	"B_T_VTOL_01_armed_F";
		_attackBoat				=	"B_Boat_Armed_01_minigun_F";
		_quadBike 				=	"B_Quadbike_01_F";
		_smallCargoTruck		=	"C_Van_01_transport_F";
		_fuelTruck				=	"C_Van_01_fuel_F";
		_car					=	"B_T_LSV_01_unarmed_F";
		_carHMG					=	"B_T_LSV_01_armed_F";
		_carAT					=	"B_T_LSV_01_AT_F";
		_Mrap					=	"B_A_MRAP_03_tna_F";
		_MrapHMG				=	"B_A_MRAP_03_hmg_tna_F";
		_MrapGMG				=	"B_A_MRAP_03_gmg_tna_F";
		_cargoTruck				=	"B_T_Truck_01_flatbed_F";
		_lightIFV				=	"Atlas_B_G_LT_01_cannon_F";
		_lightAA				=	"Atlas_B_G_LT_01_AA_F";
		_tank					=	"B_T_MBT_01_cannon_F";
		_antiAirAPC				=	"B_T_APC_Tracked_01_AA_F";
		_artilleryAPC			=	"B_T_MBT_01_arty_F";
		_wheeledIFV				=	"B_T_APC_Wheeled_01_cannon_v2_F";
		_trackedAPC				=	"B_A_APC_tracked_03_cannon_v2_tna_F";
		_tankHunter				=	"B_T_AFV_Wheeled_01_up_cannon_F";
		_smallTrHeli			=	"B_Heli_Light_01_F";
		_mediumTrHeli			=	"B_A_Heli_light_03_unarmed_F";
		_largeTrHeli			=	"B_A_Heli_Transport_02_F";
		_cargoHeli				=	"B_A_Heli_Transport_02_F";
		_smallAH				=	"B_Heli_Light_01_dynamicLoadout_F";
		_largeAH				=	"I_Heli_Attack_03_F";
		_TrPlane				=	"I_Plane_Transport_01_infantry_F";
		_CargoPlane				=	"I_Plane_Transport_01_vehicle_F";
		_plane_L1				=	"C_Plane_Civil_01_racing_F";
		_plane_L2				=	"I_Plane_Fighter_03_dynamicLoadout_F";
		_plane_L3				=	"B_Plane_Fighter_05_Stealth_F";
		_plane_L4				=	"B_Plane_Fighter_05_F";
		_plane_L5				= 	"B_Plane_Fighter_05_Cluster_F";
		_plane_L6				= 	"B_Plane_CAS_01_dynamicLoadout_F";	
		_smallBoat				=	"B_Boat_Transport_01_F";
		_sub 					=	"B_SDV_01_F";
		
	
	// Arrays

		_allUnitTypes = [
			
			_rifleman,
			_officer,
			_teamLeader, 
			_marksman, 
			_sniper, 
			_spotter, 
			_grenadier, 
			_autoRifleman, 
			_gunner, 
			_medic, 
			_repairSpecialist, 
			_engineer, 
			_guidedLAT, 
			_dumbLAT, 
			_missileSpecialistAT, 
			_missileSpecialistAA, 
			_ammoBearer, 
			_JTAC, 
			_explosivesSpecialist, 
			_uavOperator, 
			_paratrooper, 
			_heliPilot, 
			_heliCrew, 
			_fighterPilot, 
			_pilot, 
			_vehicleCrew,
			_diver,
			_diverTeamLeader,
			_diverExplosives
		
		];


		_allVehicleTypes = [
		
			_atTurret,
			_aaTurret,
			_mortar,
			_AttackCargoPlane,
			_attackBoat,
			_quadBike,
			_smallCargoTruck,
			_fuelTruck,
			_car,
			_carHMG,
			_carAT,
			_Mrap,
			_MrapHMG,
			_MrapGMG,
			_cargoTruck,
			_lightIFV,
			_lightAA,
			_tank,
			_antiAirAPC,
			_artilleryAPC,
			_wheeledIFV,
			_trackedAPC,
			_tankHunter,
			_smallTrHeli,
			_mediumTrHeli,
			_largeTrHeli,
			_cargoHeli,
			_smallAH,
			_largeAH,
			_TrPlane,
			_CargoPlane,
			_plane_L1,
			_plane_L2,
			_plane_L3,
			_plane_L4,
			_plane_L5,
			_plane_L6,			
			_smallBoat,
			_sub

		];


	_return = _allUnitTypes + _allVehicleTypes;
	
	_return	






















