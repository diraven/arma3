	...
	// Destroy MCOMS
		...
		if (BEW_v_BF4Mode) then {
			...
			_mcomObjective = [_objectivePos, _objectiveSize, _objectiveMarker, _parentTask] call BEW_FNC_destroyMCOM;
				_subObjectives pushBack _mcomObjective;
				_allVehs pushBack _mcomObjective;
	...
	// Send Message
		...
		if (({alive _x} count _towerObjectives) > 0) then {

			[HQ_WEST , format ["An enemy force and transmission tower have been detected at %1. Eliminate the enemy force and destroy the transmitters to gain control of the area.", _objectiveText]] remoteExec ["sideChat", 0];
