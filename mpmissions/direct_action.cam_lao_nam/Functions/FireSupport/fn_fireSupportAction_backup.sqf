	...
	// Main Action
		...
		call compile format ["
			...
			private _mainActionConditions = ""
				...
				and
				((currentWeapon player isKindOf ['Binocular', configFile >> 'CfgWeapons']) and (cameraView == 'Gunner'))
				and
				(_target distance screenToWorld [0.5,0.5] >= %2)
				and
				(_target distance screenToWorld [0.5,0.5] <= %3) and 
			...
			, _fireSupportData, 98, false, true, """", _mainActionConditions];
			...
			, _fireSupportData, 98, false, true, """", _outOfBoundsConditions];
			...
			, _fireSupportData, 98, false, true, """", _offlineConditions];		
			...
			_dangerCloseConditions = ""
				...
				((currentWeapon player isKindOf ['Binocular', configFile >> 'CfgWeapons']) and (cameraView == 'Gunner'))
				and
				!(_target distance screenToWorld [0.5,0.5] >= %2) and 
				!(player inArea BEW_v_HqSafeZoneMarker) and 
				(missionLevel >= %4) and
				((side player == WEST and (BEW_v_munitions >= %5)) or (side player == EAST and (BEW_v_munitions_EAST >= %5))) and 
				(vehicle player == player)
			...
			, _fireSupportData, 98, false, true, """", _dangerCloseConditions];


			_outOfRangeConditions = ""
			
				!(BEW_v_fireMissionCooldown) and 
				((currentWeapon player isKindOf ['Binocular', configFile >> 'CfgWeapons']) and 
				(cameraView == 'Gunner')) and 
				(_target distance screenToWorld [0.5,0.5] > %3) and 
				!(player inArea BEW_v_HqSafeZoneMarker) and 
				(missionLevel >= %4) and
				((side player == WEST and (BEW_v_munitions >= %5)) or (side player == EAST and (BEW_v_munitions_EAST >= %5))) and 
			...
			, _fireSupportData, 98, false, true, """", _outOfRangeConditions];
			...
			, _fireSupportData, 98, false, true, """", _notEnoughMunitionsConditions];
