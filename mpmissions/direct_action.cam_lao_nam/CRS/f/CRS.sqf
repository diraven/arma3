...
//TACTICAL POINT
if(CRS_TacticalPoint)then{waitUntil{!(isNull(findDisplay 46))};
(findDisplay 46)displayAddEventHandler["MouseButtonDown","if(inputAction""TacticalPing"">0)then{if(isNull objectParent player)then{player playAction""HandSignalPoint""};};"];};
//player playMove"AmovPercMstpSrasWrflDnon_AinvPercMstpSrasWrflDnon_Putdown"; //SHOULDER TAP
...
//player createDiarySubject["Diary","Combat Realism Scripts"];
player createDiaryRecord["Credits",["CRS Realism Scripts",
